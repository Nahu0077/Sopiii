...
async def get_bin_info(cc):
    """Get real BIN info from binlist.net API"""
    bin_num = cc[:8] if cc else '000000'
        ...
        async with aiohttp.ClientSession(timeout=timeout) as s:
            async with s.get(f"https://lookup.binlist.net/{bin_num}") as r:
                if r.status == 200:
                    ...
                    result = {
                        'brand': brand or scheme,
                        'type': card_type,
                        'level': d.get('scheme', '').upper(),
                        'bank': bank,
                        'country': country,
                        'flag': emoji
                    }
    ...
    # Fallback: basic detection from first digit
    ...
    fallback = {
        ...
        '2': ('MASTERCARD', 'CREDIT', 'WORLD', 'Sample Bank', 'US', '🇺🇸'),
    }.get(first, ('Unknown', 'Unknown', 'Unknown', 'Unknown', 'Unknown', '🌐'))
    
    result = {
        'brand': fallback[0], 'type': fallback[1], 'level': fallback[2],
        'bank': fallback[3], 'country': fallback[4], 'flag': fallback[5]
    }
    BIN_CACHE[bin_num] = result
    return result
