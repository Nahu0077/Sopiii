...
def premium_emoji(text):
    """Replace emojis with <tg-emoji> tags using safe placeholders — no double-replace"""
    if not text:
        return text
    placeholders = []
    result = text
...
async def start(event):
    ...
    base = (
        ...
        "<b>━━━━━━━━━━━━━━━━━</b>\n"
        "<b>⚡💠 𝐂𝐂 𝐂𝐨𝐦𝐦𝐚𝐧𝐝𝐬</b>\n"
        "• /sh card|mm|yy|cvv - Check single CC\n"
        "• /msh - Reply to .txt file to check cards\n"
        "<b>⚡💠 𝐒𝐢𝐭𝐞 𝐂𝐨𝐦𝐦𝐚𝐧𝐝𝐬</b>\n"
        "• /siteran - Check all sites & remove dead\n"
    ...
    base += "<b>━━━━━━━━━━━━━━━━━</b>\n"
    base += "<b>⚠️ Only premium users can use this bot.</b>" if not is_prem else "<b>✅ You have premium access.</b>"
...
async def sh_noargs(event):
    if not is_premium(event.sender_id):
        await event.reply(premium_emoji("<b>⚡ 𝗔𝗰𝗰𝗲𝘀𝘀 𝗗𝗲𝗻𝗶𝗲𝗱!</b>\n\n<b>⚡ 𝗚𝗲𝘁 𝗽𝗿𝗲𝗺𝗶𝘂𝗺:</b> Redeem a key with <code>/redeem nschk-xxx</code>"), parse_mode='html')
        return
...
🤖 <b>Bot By: <a href="tg://user?id=7603003996">ㅤ Nahid Hossen</a></b>"""
        ...
        await sm.edit(premium_emoji(f"❌ Error: {e}"))
...
async def msh(event):
    ...
    if not load_proxies():
        await event.reply(premium_emoji("❌ No proxies available."), parse_mode='html'); return
    sm = await event.reply(premium_emoji("🔄 Processing your file..."))
    fp = await reply.download_media()
    async with aiofiles.open(fp, 'r', encoding='utf-8', errors='ignore') as f:
        content = await f.read()
    cards = extract_cc(content)
    if not cards:
        await sm.edit(premium_emoji("😡 No valid cards found in file."))
        os.remove(fp); return
    if len(cards) > 5000:
        cards = cards[:5000]
    os.remove(fp)
...
async def proxy_check(event):
    ...
    if not proxies:
        await event.reply(premium_emoji("❌ No proxies available."), parse_mode='html'); return
    sm = await event.reply(premium_emoji(f"🔥 Checking {len(proxies)} proxies in batches of 50..."))
    alive, dead = [], []
    bs = 50
    try:
...
async def siteran(event):
    ...
    sm = await event.reply(premium_emoji(f"🔥 Checking {len(sites)} sites..."))
    alive_s, dead_s = [], []
    bs = 10
    try:
...
async def restart(event):
    ...
    await event.reply(premium_emoji("🔄 Restarting bot..."))
