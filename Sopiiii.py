...
PREMIUM_EMOJI_IDS = {
    ...
    "💎": "6023660820544623088",   # ✨
    "🛡️": "5999340396432333728",   # 🔥 shield
    "🔰": "6037570896766438989",   # ❌ beginner
    "💫": "5971944878815317190",   # 💳 dizzy
    "🔮": "6026367225466720832",   # ⚡ crystal ball
    "💸": "5971944878815317190",   # 💳 money wings
    "✨": "6023660820544623088",   # ✨ sparkles
    "🎉": "6282977077427702833",   # 🚀 party popper
    "🔔": "6023660820544623088",   # ✨ bell
    "💡": "5999340396432333728",   # 🔥 lightbulb
    "📌": "6066395745139824604",   # 📦 pin
    "🃏": "5974235702701853774",   # 🟠🟡🟢 Triple Ring Loader
    "📂": "5971837723676249096",   # 🌀 Neon Circle Rings
    "🔀": "6285315214673975495",   # ➡️ Neon Arrow Right
    "➕": "6023660820544623088",   # ✨
    "🔍": "6026367225466720832",   # ⚡
    "🗑️": "6037570896766438989",   # 💀 delete
    "🔢": "6066395745139824604",   # 🎀 Neon Pink Bow
    "🧹": "5971944878815317190",   # 💫
    "📡": "4949560993840629085",   # 🧠 Golden Maze
    "🔑": "5999340396432333728",   # 🔥 key
    "👑": "6282977077427702833",   # 🎉 Color Confetti
    "😡": "5999340396432333728",   # 🔥 angry face
    "🫆": "6023660820544623088",   # ✨
    "🤷": "6057466460886799210",   # 😼 shrug
    "💀": "6037570896766438989",   # 💀 skull
}
...
KEYS_FILE = 'keys.txt'
GROUPS_FILE = 'groups.txt'  # Allowed group IDs
...
# ===== GROUP FUNCTIONS =====
def load_groups():
    """Load allowed group IDs from file"""
    if not os.path.exists(GROUPS_FILE):
        os.makedirs(os.path.dirname(GROUPS_FILE) if os.path.dirname(GROUPS_FILE) else '.', exist_ok=True)
...
def is_allowed_group(chat_id):
    """Check if chat is an allowed group"""
    chat_id = str(chat_id)
    groups = load_groups()
...
async def check_card_with_retry(card, sites, proxies, max_retries=5):
    """Check a card with automatic retry — Tor fallback when proxies fail"""
    last_result = None
    if not sites:
        sites = [FALLBACK_SITE]
    if not proxies:
        proxies = ['']  # Empty = Tor fallback
    ...
    # Final fallback: always-working store via Tor
    result = await check_card(card, FALLBACK_SITE, '')
    if result.get('status') in ['Charged', 'Approved']:
        return result
    return {'status': 'Dead', 'message': result.get('message', 'Max retries exceeded'), 'card': card, 'gateway': result.get('gateway', 'Unknown'), 'price': result.get('price', '-'), 'site': FALLBACK_SITE}
...
async def test_site(site, proxy):
    ...
    try:
        # Try with proxy first, then without (Tor fallback)
        for p in [proxy, '']:
            api_url = f"{CHECKER_API_URL}/?{test_card}&url={site}&proxy={p}"
            timeout = aiohttp.ClientTimeout(total=60)
            async with aiohttp.ClientSession(timeout=timeout) as session:
                async with session.get(api_url) as resp:
                    raw = await resp.json(content_type=None)
            response_msg = raw.get('Response', '').lower()
            if not is_dead_site_error(response_msg) and 'error' not in response_msg:
...
async def start(event):
    ...
    base_text = (
        ...
        "📂 <code>/msh</code> — Mass check cards (.txt file)\n"
        "🌐 <code>/siteran</code> — Test & remove dead sites\n"
        "🔀 <code>/proxy</code> — Test & remove dead proxies\n"
        "➕ <code>/addproxy</code> — Add proxies (one per line)\n"
        "🔍 <code>/chkproxy</code> — Check single proxy\n"
        "🗑️ <code>/rmproxy</code> — Remove single proxy\n"
        "🔢 <code>/rmproxyindex 1,2,3</code> — Remove by index\n"
        "🧹 <code>/clearproxy</code> — Remove ALL proxies\n"
        "📋 <code>/getproxy</code> — View all proxies\n"
        "📡 <code>/ping</code> — Check checker API status\n"
        "🔑 <code>/redeem nschk-xxx</code> — Activate premium key\n"
        "</blockquote>\n"
    )
    ...
    if user_is_admin:
        base_text += (
            ...
            "━━━━━━━━━━━━━━━━━━━━━━</pre>\n"
            "<blockquote>"
            "👑 <code>/genkey [count] [days]</code> — Generate keys\n"
            "👑 <code>/addadmin USER_ID</code> — Add admin\n"
            "👑 <code>/adminlist</code> — View admins\n"
            "👑 <code>/rm SITE_URL</code> — Remove site\n"
    ...
    if not user_is_premium:
        base_text += "<b>⚠️ Only premium users can use this bot.</b>"
    else:
        base_text += "<b>✅ You have premium access. Enjoy!</b>"
    
    await event.reply(premium_emoji(base_text), parse_mode='html')

@bot.on(events.NewMessage(pattern=r'^/sh$'))
async def single_cc_noargs(event):
    """Check /sh without arguments"""
    user_id = event.sender_id
    
    # Access: Premium users allowed everywhere. Free users need approved group.
    if not is_premium(user_id) and not (event.is_group and is_allowed_group(event.chat_id)):
        await event.reply(premium_emoji(
            "<b>⚡ 𝗔𝗰𝗰𝗲𝘀𝘀 𝗗𝗲𝗻𝗶𝗲𝗱!</b>\n\n"
            "<b>⚡ 𝗚𝗲𝘁 𝗽𝗿𝗲𝗺𝗶𝘂𝗺:</b> Redeem a key with <code>/redeem nschk-xxx</code>"
        ), parse_mode='html')
        return
    await event.reply(premium_emoji(
        "<b>⚡ 𝗡𝗼 𝗖𝗖 𝗳𝗼𝘂𝗻𝗱!</b>\n\n"
...
async def single_cc_check(event):
    """Check a single CC"""
    if event.is_group and not is_allowed_group(event.chat_id):
        return
    user_id = event.sender_id
...
async def check_command(event):
    ...
    if not event.reply_to_msg_id:
        await event.reply(premium_emoji(
            "<b>⚡ 𝗨𝘀𝗮𝗴𝗲:</b>\n\n"
            "<b>⚡ 𝗦𝗲𝗻𝗱 𝗮 .𝘁𝘅𝘁 𝗳𝗶𝗹𝗲 𝘄𝗶𝘁𝗵 𝗖𝗖𝘀</b>\n"
            "<b>⚡ 𝗥𝗲𝗽𝗹𝘆 𝘁𝗼 𝘁𝗵𝗲 𝗳𝗶𝗹𝗲 𝘄𝗶𝘁𝗵 /msh</b>\n\n"
            "<b>⚡ 𝗙𝗼𝗿𝗺𝗮𝘁:</b> cc|mm|yy|cvv (𝗼𝗻𝗲 𝗽𝗲𝗿 𝗹𝗶𝗻𝗲)"
        ))
        return
    ...
    if not reply_msg.file or not reply_msg.file.name.endswith('.txt'):
        await event.reply(premium_emoji(
            "<b>⚡ 𝗣𝗹𝗲𝗮𝘀𝗲 𝘀𝗲𝗻𝗱 𝗮 .𝘁𝘅𝘁 𝗳𝗶𝗹𝗲 𝘄𝗶𝘁𝗵 𝗖𝗖𝘀!</b>\n\n"
            "<b>⚡ 𝗥𝗲𝗽𝗹𝘆 𝘁𝗼 𝘁𝗵𝗲 𝗳𝗶𝗹𝗲 𝘄𝗶𝘁𝗵 /msh</b>"
        ))
        return
...
async def site_command(event):
    """Check all sites and remove dead ones using API tests"""
    if event.is_group and not is_allowed_group(event.chat_id):
        return
    user_id = event.sender_id
...
async def ping_command(event):
    ...
    if not is_premium(user_id):
        await event.reply(premium_emoji("❌ <b>Access Denied</b>\n\nOnly premium users can use this command."), parse_mode='html')
        return
    msg = await event.reply(premium_emoji("🔄 <b>Pinging checker API...</b>"), parse_mode='html')
    try:
        timeout = aiohttp.ClientTimeout(total=15)
        async with aiohttp.ClientSession(timeout=timeout) as session:
            async with session.get(f"{CHECKER_API_URL}/?5154623245618097|03|2032|156&url=https://fxdesigns-8213.myshopify.com") as resp:
                raw = await resp.json(content_type=None)
                resp_msg = raw.get('Response', 'no response')
                price = raw.get('Price', '-')
                await msg.edit(premium_emoji(
                    f"✅ <b>Checker API is ALIVE!</b>\n\n"
                    f"<blockquote>Response: {resp_msg}</blockquote>\n"
                    f"<blockquote>💰 Price: {price}</blockquote>\n"
                    f"<blockquote>⚡ Speed: {raw.get('Time', '?')}</blockquote>\n"
                    f"<blockquote>🌐 API: <code>{CHECKER_API_URL}</code></blockquote>"
                ), parse_mode='html')
    except Exception as e:
        await msg.edit(premium_emoji(f"❌ <b>Checker API is DOWN!</b>\n\nError: {e}"), parse_mode='html')

@bot.on(events.NewMessage(pattern='/restart'))
async def restart_command(event):
    user_id = event.sender_id
    if not is_admin(user_id):
        await event.reply(premium_emoji("⚡ <b>Admin only command!</b>"), parse_mode='html')
        return
    await event.reply(premium_emoji("🔄 <b>Restarting bot...</b>"), parse_mode='html')
    os.execl(sys.executable, sys.executable, *sys.argv)

@bot.on(events.NewMessage(pattern='/redeem'))
async def redeem_command(event):
    """Redeem a premium key: /redeem Json-XXXXX"""
    user_id = event.sender_id
    
    try:
        args = event.message.text.split(' ', 1)
        if len(args) < 2:
            await event.reply(premium_emoji("\u26a1 <b>Usage:</b> <code>/redeem nschk-xxxxxxxxxxxxxxx</code>"), parse_mode='html')
            return
        
        key = args[1].strip()
        if not key.startswith('nschk-'):
            await event.reply(premium_emoji("\u26a1 <b>\U0001d5d1\U0001d5d9\U0001d5d4\U0001d5d9\U0001d5e4\U0001d5f8\U0001d5f2\U0001d5e0\U0001d5ec \U0001d5c5\U0001d5e2\U0001d5f7\U0001d5d9\U0001d5d4\U0001d5e5\U0001d5e7\U0001d5ec\U0001d5e6\U0001d5d1\U0001d5ea\U0001d5ea\U0001d5ec!</b>\n\nUse: <code>/redeem nschk-xxxxxxxxxxxxxxx</code>"), parse_mode='html')
            return
        
        success, msg = redeem_key(key, user_id)
            }
            await event.reply(premium_emoji(fail_msgs.get(msg, f'\u26a1 <b>\U0001d5d1\U0001d5d9\U0001d5d4\U0001d5d9\U0001d5e4\U0001d5f8\U0001d5f2\U0001d5e0 \U0001d5d8\U0001d5e2\U0001d5f7\U0001d5d3\U0001d5e6\U0001d5f6\U0001d5d9\U0001d5ce\U0001d5e7\U0001d5f2\U0001d5f7\U0001d5d4\u26a1\n\n\u26a1 {msg}')), parse_mode='html')
...
async def genkey_command(event):
    """Generate premium keys - Admin Only: /genkey [count] [days]"""
    user_id = event.sender_id
    if not is_admin(user_id):
        await event.reply(premium_emoji("\u26a1 <b>\U0001d5d0\U0001d5d4\U0001d5dc\U0001d5d9\U0001d5d9 \U0001d5e0\U0001d5ee\U0001d5f2\U0001d5f7 \U0001d5d6\U0001d5ee\U0001d5e4\U0001d5e4\U0001d5ee\U0001d5f2\U0001d5eb!</b>"), parse_mode='html')
        return
    ...
    try:
        ...
        key_lines = "\n".join([f"\u2523 <code>{key}</code>" if i < count-1 else f"\u2517 <code>{key}</code>" for i, key in enumerate(keys)])
        
        msg = f"""<b>\U0001d5d8\U0001d5d9\U0001d5de\U0001d5e2 \U0001d5d0\U0001d5d9\U0001d5de\U0001d5e5\U0001d5e1\U0001d5e6\U0001d5e5\U0001d5d9\U0001d5d4 \u2705</b>
<b>\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501</b>
\u2523 \U0001d7d6\U0001d7e0\U0001d7e6\U0001d7f4\U0001d7f7 \u27a1 {count}
\u2523 \U0001d7d3\U0001d7e3\U0001d7e2\U0001d7f1 \u27a1 {plan}
\u2523 \U0001d7d8\U0001d7d9\U0001d7de\U0001d7e2 \u2705
{key_lines}

<blockquote>\U0001d7d4\U0001d7e2\U0001d7e5\U0001d7e2\U0001d7e6 \U0001d7e5\U0001d7d9\U0001d7d4\U0001d7d9\U0001d7d4\U0001d7e0 \U0001d7f0\U0001d7d8\U0001d7f1\U0001d7e2 /redeem [key] \u26a1</blockquote>"""
...
async def addgc_command(event):
    """Add a group to allowed list - Admin Only: /addgc GROUP_ID"""
    user_id = event.sender_id
    if not is_admin(user_id):
        await event.reply(premium_emoji("⚡ <b>𝗔𝗱𝗺𝗶𝗻 𝗼𝗻𝗹𝘆 𝗰𝗼𝗺𝗺𝗮𝗻𝗱!</b>"), parse_mode='html')
        return
    try:
        args = event.message.text.split(' ', 1)
        if len(args) < 2:
            await event.reply(premium_emoji("⚡ <b>Usage:</b> <code>/addgc GROUP_ID</code>\n\nExample: <code>/addgc -1004305447869</code>"), parse_mode='html')
            return
        group_id = args[1].strip()
        if add_group(group_id):
            await event.reply(premium_emoji(f"✅ <b>Group approved!</b>\n\n<code>{group_id}</code> has been added to the allowed groups list.\nThe bot will now respond in this group."), parse_mode='html')
        else:
            await event.reply(premium_emoji("❌ Group is already in the allowed list."), parse_mode='html')
    except Exception as e:
        await event.reply(premium_emoji(f"❌ Error: {e}"), parse_mode='html')
