...
def premium_emoji(text):
    """Replace emoji placeholders with regular Unicode emojis that render everywhere"""
    if not text:
        return text
    return text
...
async def check_command(event):
    ...
    await status_msg.edit(premium_emoji(
        f"✨ <b>Starting check for {total_cards} cards...</b>\n\n"
        f"⚡ Status: Running | 💳 Total: {total_cards}"
    ))
...
<b>⚡ Enjoy your premium experience!</b>"""), parse_mode='html')
            ...
            fail_msgs = {
                'Invalid key': '<b>⚡ Redemption Failed!</b>\n\n⚡ Key already redeemed or does not exist',
                'Key already used': '<b>⚡ Redemption Failed!</b>\n\n⚡ Key already redeemed or does not exist',
            }
            await event.reply(premium_emoji(fail_msgs.get(msg, f'<b>⚡ Redemption Failed!</b>\n\n⚡ {msg}')), parse_mode='html')
