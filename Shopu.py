...
PREMIUM_EMOJI_IDS = {
    ...
    "💎": "6023660820544623088",
    "🔒": "6026367225466720832",
    "⏸": "6001440193058444284", "▶": "6285315214673975495",
    "⚠": "5420323339723881652", "🗑": "6037570896766438989",
    "🛡": "5999340396432333728",
    "1️⃣": "6023660820544623088", "2️⃣": "5971837723676249096",
    "3️⃣": "5974235702701853774",
}
...
async def check_force_join(uid):
    ...
    try:
        ...
        return False  # Lock if check fails
...
# ========== BIN INFO (Real-time lookup) ==========
BIN_CACHE = {}

async def get_bin_info(cc):
    """Get real BIN info — tries 2 APIs, then detailed fallback"""
    bin_num = cc[:6] if len(cc) >= 6 else cc + '0'*(6-len(cc))
    ...
    for api_url in [
        ...
        f"https://bins.ws/bins/{bin_num}?format=json",
    ]:
        try:
            timeout = aiohttp.ClientTimeout(total=5)
            async with aiohttp.ClientSession(timeout=timeout) as s:
                async with s.get(api_url, headers={'Accept':'application/json'}) as r:
                    if r.status == 200:
                        d = await r.json()
                        scheme = d.get('scheme', d.get('network', d.get('brand',''))).upper()
                        if scheme:
                            card_type = d.get('type', 'CREDIT').upper()
                            brand = (d.get('brand','') or '').upper()
                            bank = d.get('bank', {}).get('name') if isinstance(d.get('bank'), dict) else d.get('bank', 'Unknown')
                            if not bank: bank = d.get('issuer', d.get('issuing_bank', 'Unknown'))
                            country_data = d.get('country', {})
                            country = country_data.get('name', 'Unknown') if isinstance(country_data, dict) else str(country_data)
                            emoji = country_data.get('emoji', '🌐') if isinstance(country_data, dict) else '🌐'
                            currency = country_data.get('currency', 'USD') if isinstance(country_data, dict) else 'USD'
                            result = {
                                'scheme': scheme, 'type': card_type,
                                'brand': brand or scheme, 'bank': bank,
                                'country': country, 'flag': emoji,
                                'currency': currency,
                                'bin_text': f"{scheme}-{card_type}-{brand or scheme}"
                            }
                            break
        except:
            continue
    ...
    # Detailed fallback based on IIN ranges
    first = cc[:1] if cc else '4'
    first2 = cc[:2] if len(cc) >= 2 else '00'
    scheme = card_type = level = 'UNKNOWN'
    bank, country, flag, currency = 'Unknown', 'Unknown', '🌐', 'USD'
    
    if first == '4':
        scheme = 'VISA'
        card_type = 'CREDIT'
        level = 'CLASSIC'
        if first2 in ('40','41','48','49'): level = 'CLASSIC'
        elif first2 == '44': level = 'ELECTRON'
        elif first2 == '42': level = 'BUSINESS'
        elif first2 == '45': level = 'GOLD'
        elif first2 == '47': level = 'PREMIER'
        bank, country, flag = 'VISA CARD ISSUER', 'VARIOUS', '🌐'
    elif first == '5':
        scheme = 'MASTERCARD'
        card_type = 'CREDIT'
        level = 'STANDARD'
        if first2 in ('51','52','53','54','55'): level = 'STANDARD'
        elif first2 == '50': level = 'GOLD'
        elif cc[:4] in ('2221','2720','5100','5500'): level = 'WORLD'
        bank, country, flag = 'MASTERCARD ISSUER', 'VARIOUS', '🌐'
    elif first == '3':
        if first2 in ('34','37'): scheme = 'AMEX'; card_type = 'CHARGE'; level = 'PLATINUM'; bank, country, flag = 'AMERICAN EXPRESS', 'UNITED STATES', '🇺🇸'; currency = 'USD'
        elif cc[:4] in ('3000','3050'): scheme = 'DINERS'; card_type = 'CHARGE'; level = 'CLASSIC'; bank, country, flag = 'DINERS CLUB', 'UNITED STATES', '🇺🇸'; currency = 'USD'
        else: scheme = 'AMEX'; card_type = 'CREDIT'; level = 'STANDARD'; bank = 'AMERICAN EXPRESS'; country = 'VARIOUS'; flag = '🌐'
    elif first == '6':
        scheme = 'DISCOVER'
        card_type = 'CREDIT'
        level = 'STANDARD'
        if first2 == '60': level = 'DISCOVER'
        elif first2 == '61': level = 'DISCOVER MORE'
        elif first2 == '62': level = 'DISCOVER IT'
        elif first2 == '64': level = 'DISCOVER MILES'
        elif first2 == '65': level = 'DISCOVER CHROME'
        bank, country, flag = 'DISCOVER BANK', 'UNITED STATES', '🇺🇸'
        currency = 'USD'
    elif first == '2':
        scheme = 'MASTERCARD'
        card_type = 'CREDIT'
        level = 'WORLD'
        if first2 == '22': level = 'WORLD ELITE'
        elif first2 == '23': level = 'WORLD'
        elif first2 == '24': level = 'PLATINUM'
        elif first2 == '25': level = 'GOLD'
        elif first2 == '26': level = 'STANDARD'
        elif first2 == '27': level = 'SELECT'
        bank, country, flag = 'MASTERCARD ISSUER', 'VARIOUS', '🌐'
    
    result = {
        'scheme': scheme, 'type': card_type, 'brand': level,
        'bank': bank, 'country': country, 'flag': flag,
        'currency': currency,
        'bin_text': f"{scheme}-{card_type}-{level}"
    }
    BIN_CACHE[bin_num] = result
    return result
...
<b>━━━━━━━━━━━━━━━━━</b>
<b>🎯💠 𝐁𝐈𝐍 𝐈𝐧𝐟𝐨</b>
<pre>𝗜𝗻𝗳𝗼 ➜ {binfo['bin_text']}
𝗜𝘀𝘀𝘂𝗲𝗿 ➜ {binfo['bank']} 🏛
𝗖𝗼𝘂𝗻𝘁𝗿𝘆 ➜ {binfo['country']} {binfo['flag']}
𝗖𝘂𝗿𝗿𝗲𝗻𝗰𝘆 ➜ {binfo['currency']}</pre>
<b>━━━━━━━━━━━━━━━━━</b>
...
async def send_final(uid, results):
    ...
    h, m, s = elapsed//3600, (elapsed%3600)//60, elapsed%60
    gw = results['charged'][0]['gateway'] if results['charged'] else (results['approved'][0]['gateway'] if results['approved'] else 'Unknown')
    hits_text = results.get('hits_file', 'No hits')
...
🤖 <b>Bot By: <a href="tg://user?id=7603003996">ㅤ Nahid Hossen</a></b>"""
    ...
    # Also send as .txt file if there are hits
    if hits_text and hits_text != 'No hits':
        try:
            import tempfile
            with tempfile.NamedTemporaryFile(mode='w', suffix='.txt', delete=False, encoding='utf-8') as f:
                f.write(hits_text)
                fpath = f.name
            await bot.send_file(uid, fpath, caption=premium_emoji("📋 Hits File"))
            os.remove(fpath)
        except: pass
...
async def start(event):
    ...
    # Force join check BEFORE showing menu
    if not is_prem:
...
async def msh(event):
    ...
    hits = []
    all_results = []  # NEW: all cards saved here
    idx = 0
    sites = load_sites()[:]
    proxies = load_proxies()[:]
    BATCH = 10  # Concurrent checks
    for bi in range(0, len(cards), BATCH):
        batch_cards = cards[bi:bi+BATCH]
        batch_strs = [f"{c[0]}|{c[1]}|{c[2]}|{c[3]}" for c in batch_cards]
        
        # Check all 10 concurrently
        tasks = [check_card_with_retry(cstr, sites, proxies, 5) for cstr in batch_strs]
        batch_results = await asyncio.gather(*tasks, return_exceptions=True)
        
        for ci, (cstr, r) in enumerate(zip(batch_strs, batch_results)):
            if isinstance(r, Exception):
                results['dead'].append({'status':'Dead','message':str(r),'card':cstr,'gateway':'Unknown','price':'-'})
                all_results.append(f"❌ {cstr} | DEAD | {str(r)[:50]} | Unknown")
                continue
            st = r.get('status','Dead')
            if st == 'Charged':
                results['charged'].append(r)
                hits.append(f"✅ {cstr} | CHARGED | {r.get('price','-')}")
                all_results.append(f"✅ {cstr} | CHARGED | {r.get('price','-')} | {r.get('gateway','-')}")
                await send_hit(uid, r, 'Charged', '')
            elif st == 'Approved':
                results['approved'].append(r)
                hits.append(f"🔥 {cstr} | LIVE | {r.get('price','-')}")
                all_results.append(f"🔥 {cstr} | LIVE | {r.get('price','-')} | {r.get('gateway','-')}")
                await send_hit(uid, r, 'Approved', '')
            else:
                results['dead'].append(r)
                all_results.append(f"❌ {cstr} | DEAD | {r.get('message','')[:50]} | {r.get('gateway','-')}")
    ...
    results['all_file'] = '\n'.join(all_results)  # NEW: full results
...
async def ping(event):
    ...
    if not is_premium(uid):
        await send_force_join(event)
        return
    msg = await event.reply(premium_emoji("📡 Pinging API..."), parse_mode='html')
    try:
